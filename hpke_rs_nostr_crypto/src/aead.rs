...
use aes_gcm::Aes128Gcm as RC_Aes128Gcm;
...
macro_rules! implement_aead {
    ($name_seal: ident, $name_open: ident, $name:ident, $algorithm:ident) => {
        ...
        pub(crate) fn $name_open(
            ...
            let nonce_length = HpkeNostrCrypto::aead_nonce_length(alg);
            if nonce.len() != nonce_length {
                return Err(Error::AeadInvalidNonce);
            }
            let tag_length = HpkeNostrCrypto::aead_tag_length(alg);
